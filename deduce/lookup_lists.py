...
INTERFIX_SURNAMES = set(line.strip().split(" ")[-1] for line in read_list("achternaammetvv.lst"))
...
WHITELIST = set(line.lower() for line in MEDTERM.union(TOP1000, STOPWORDS) if len(line) >= 2)
...
INSTITUTIONS = set(FILTERED_INSTITUTIONS).difference(WHITELIST)
...
RESIDENCES = set(re.sub("\(.+\)", "", residence) for residence in RESIDENCES)
