...
class TestBoosterRecall:
    ...
    def test_overlap_resolve_year(self, model_with_recall_boost, text, expected_texts):
        """
        In the specific case of space separated dates that contain a year the year gets
        annotated again by the date_dmy_1 annotator.

        Due to length priority this extra annotation should be removed, which is tested
        here.
        """
    ...
    def test_current_year_replace(self, model_with_recall_boost):
        """The maximum year is capped in the year only regex, just in case deduce will
        last a long time this test will fail if the year surpasses the maximum year that
        is defined currently -> 2029."""
